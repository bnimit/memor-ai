...
### Added
- **`memor request-anatomy` — where a request's tokens actually go, and the ceiling that puts on compression.** "Why is compression only 7%" kept being answered with theories about the compressor when the answer is arithmetic about the request. Measured across 6 large real sessions: tool-use arguments are **42.7%** (the code being written, where eliding corrupts edits), tool results **33.0%** (the only slice memor rewrites), conversation text **23.9%** (rewriting it re-forms the provider's cached prefix), images **0.4%**. At the observed 43.3% rate on tool output, the ceiling for a whole request is **14.3%** — so a competitor claiming more on this traffic shape is measuring a different denominator, typically log-heavy or document-heavy workloads. The module prices images the way a provider does rather than tokenising their base64: a 161 KB PNG counts as 113,367 tokens if the encoded string is fed to a tokenizer, which put images at 31% of a session and made them look like the largest available prize. They are 0.4%, a 70x error, caught by noticing the estimate implied far more tokens than the provider ever billed. A test pins the correct pricing.
- **README results table now names the command that reproduces each figure**, and leads with compression on tool output (43.3%), answer-critical retention (96.2% against truncation's 50.3%), and retrieval accuracy (95.0% on LongMemEval). The request-anatomy breakdown is published alongside, so the small blended figure is explained by arithmetic rather than left to look like a weak compressor.

- **Source compression is now on by default** (`compress_source`, override with `MEMOR_COMPRESS_SOURCE=0`). File contents are 46.2% of tool-payload mass and were never touched, which is the single largest reason realized compression sat at 7.3%. Gated on the retention measurement rather than on tokens alone: under the production rule — the newest read of each file stays byte-exact, only superseded copies are skeletonized — **20.5% saved at 96.2% answer-critical retention** over 132 grounded cases. Skeletonizing every source payload unconditionally would save 18.1% at 89.4%, so the recency rule both saves more and damages less; it is doing real work rather than being a safety token. For context, truncation to a comparable budget retains 50.3%.

- **`memor eval-retention` — answer-critical retention, the measurement every compression figure was missing.** Token savings alone cannot say whether the compressed result is still usable, and memor has published savings with no accuracy gate at all. Competing tools answer this with gold-answer containment over QA datasets: compress a document holding a known answer, check the answer string survived. A coding agent has no QA labels, but it has better ground truth — what the agent actually did next. When a later `Edit` quotes text, that text must have survived for the edit to be possible, so the transcript supplies its own labels. Deterministic: no LLM, no judge that can drift between runs. Reports retention and savings together, because reporting only one invites publishing the easy half. On 166 grounded cases from local transcripts: **memor retains 100% at its current settings**, against **50.0% for truncation to the same 35% budget** — the naive baseline competitors compare against. The first version of this harness reported 8.9% retention at 0% compression, which is impossible; 358 of 393 quoted fragments were never in the payload they were scored against, because the agent edits a file on disk that has moved past the read we captured. Cases are now discarded unless the evidence is provably present before compression, and a test pins the identity case at 100%.
...
### Fixed
- **`memor install-compress-hook` failed on an editable install with "memor-posttool-compress not found on PATH".** The entry point is declared in `pyproject.toml`, but a pipx environment created before it was added never generated the script, and the error told the user to reinstall without saying that an editable install needs `pipx install --force -e .` to regenerate entry points. The command now works once the environment is refreshed; the underlying gap is that adding an entry point does not retroactively create it in an existing install.
- **README overstated compression by leading with the flattering denominator.** The table quoted 43.3%, which is the rate over payloads the compressor engages, not over tool output as a whole. Across all tool output the figure is **11.7%**, and a whole request comes down by **3.9%**. Both are now shown, with the subset labelled as a subset — the 43.3% figure describes what the compressor does when it acts, and cannot be read as a saving on anyone's bill.

- **A noisy prompt-size band could veto a well-powered result, so the recall panel could never move.** `verdict()` returned `no_effect` whenever the strata disagreed in sign — sound protection against composition, but it weighted every scored band equally. Tool calls per episode have a standard deviation larger than their mean, so a band of 250 episodes cannot resolve a difference below roughly a third. On the real store three of the four bands sat under their own detectable floor (observed 17.0%/-15.9%/-17.8% against floors of 32.0%/30.6%/43.9%), so their signs were coin flips, while the aggregate showed **+22.5% against a 13.5% floor** — detectable, and vetoed by noise. "Not enough data yet" was the wrong explanation: more data would not have changed the rule. Each stratum now records its minimum detectable effect, and only a stratum that clears its own floor gets a vote. Genuine reversals between two significant bands still veto. The verdict on real data moves from `no_effect` to **`saves`**, reporting +22.5% aggregate and +20.3% project-adjusted, with the one powered band (long prompts) at +60.8%.

- **The hook applied a stricter similarity gate than recall itself, and returned nothing.** `DEFAULT_MIN_SIMILARITY` was lowered to -0.05 because these static embeddings score a good match near 0.05, putting a zero floor inside the noise band. The hook resolved its own default with a hardcoded `"0.0"` fallback, so it never followed: every hook-served recall was gated more strictly than every other caller. Measured on the real store, `recall()` returned 6 hits at 0.789 for a genuine prompt while the hook answered *"no relevant memories for project ... yet"* for the same query against the same data. The failure is silent — a user sees an empty dashboard and concludes memory is broken. After the fix the same prompt returns 2,142 characters of context.
- **Project directories containing a space were filed under the wrong name.** Claude encodes path separators as dashes, so `Gesture App` and `Gesture-App` produce an identical directory name. The decoder resolved the ambiguity by walking the filesystem but only ever rejoined tokens with a dash, so `/Users/.../Projects/Gesture App` decoded to `.../Gesture/App` and every transcript was ingested under a project called `App`. On one real day that project received 131 of 203 prompts, every recall returned `no_hits`, and the store held zero artifacts under its real name. Dash is still tried first, so `seo-team` cannot be shadowed by a hypothetical `seo team` directory.
