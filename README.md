...
```
  ...
  | inspector| browser   | from UI + | relation- |
  | w/ score | filter by | view past | ships     |
  | breakdown| project/  | runs with | outgoing +|
  | per hit  | kind/text | metrics   | incoming  |
  +----------+-----------+-----------+-----------+
```
...
| | memor-ai | Graphiti (Zep) | claude-mem | mem0 |
|---|---|---|---|---|
| **Target use case** | Coding agents | General knowledge graphs | Claude Code sessions | General chatbot memory |
| **Infrastructure** | SQLite (single file) | Neo4j + hosted service | SQLite | Cloud API or Qdrant + Postgres |
| **Eval harness** | Built-in, 4 baselines + ablation | None | None | None |
| **Distillation** | Two-step (82% cost reduction) | LLM-only (full input) | None (raw storage) | LLM-only |
| **Graph model** | Lightweight edges + recursive CTE | Full Neo4j property graph | None | None |
| **LLM-free mode** | Yes (extractive fallback) | No | Yes (raw only) | No |
| **Scoped retrieval** | Project + time + kind | Per-user | Per-project | Per-user |
| **Setup** | npm install (auto-venv) | Docker + Neo4j + config | pip install | Cloud signup or Docker |
| **Embeddings** | Local (offline, free) | OpenAI API (paid) | Local | OpenAI API (paid) |

### Where memor-ai wins

1. **Measured, not claimed.** Every feature has a measured delta. The eval harness is built in. Other tools say "better retrieval" — we show recall@k numbers.

2. **Zero infrastructure.** One SQLite file. No Docker, no Neo4j, no cloud account. npm install and you're done.

3. **Coding-agent native.** Built for the specific patterns of agentic coding: session transcripts, decisions, bugfixes, architecture context. Not a general chatbot memory bolted onto dev tools.

4. **Cost-aware distillation.** Two-step pipeline means 82% less LLM input. Extractive-only mode means $0 when you don't have an API key.

5. **Actually offline.** Local embeddings (sentence-transformers). Local storage (SQLite). Optional LLM for distillation. Everything else works offline.
