...
class SqliteStore:
    ...
    def _migrate_cache_creation_tokens(self):
        """Add upstream_cache_creation_tokens to ledgers that predate it.

        Without it, a request that busted the prompt cache is indistinguishable
        from one that never used caching: both report a cache read of zero. The
        distinction is the whole question compression has to answer, since
        rewriting a cached prefix converts cheap cache reads into full-price
        cache writes.
        """
        cols = [r[1] for r in self.db.execute(
            "PRAGMA table_info(proxy_savings)").fetchall()]
        if "upstream_cache_creation_tokens" not in cols:
            try:
                self.db.execute(
    ...
    def update_proxy_usage(self, row_id: int | None, usage: dict) -> None:
        """Fill in upstream token counts once a streamed response has ended.

        Streaming forced the ledger row to be written before the provider had
        reported anything, so the usage columns were hardcoded to None and
        stayed that way for every real request. The row is written first (so a
        dropped connection still records that compression happened) and
        completed here. Never raises: this is bookkeeping.
        """
        if row_id is None or not usage:
            return
        fields = [k for k in (
            "upstream_input_tokens", "upstream_cache_read_tokens",
            "upstream_cache_creation_tokens", "upstream_output_tokens",
        ...
        try:
            self.db.execute(
                f"UPDATE proxy_savings SET {assignments} WHERE id=?",
                [usage[f] for f in fields] + [row_id])
            self.db.commit()
        except sqlite3.Error as exc:
